       ...
       <div class="col-sm-5">
            ...
            <div class="kt-portlet__head">
              <div class="kt-portlet__head-label">
                <div class="panel-heading">
                  <strong>Menu Order (Active)</strong><br> 
                  <span id='menu-saved-info' style="display:none" class='pull-right text-success'><i
            ...
            <div class="kt-portlet__body">
              <div class="panel-body clearfix">
                <ul class='draggable-menu draggable-menu-active'>
                   @foreach($menu_active as $menu) @php $privileges = DB::table('cms_menus_privileges') ->join('cms_privileges','cms_privileges.id','=','cms_menus_privileges.id_cms_privileges') ->where('id_cms_menus',$menu->id)->pluck('cms_privileges.name')->toArray(); @endphp
                   <li data-id='{{$menu->id}}' data-name='{{$menu->name}}'>
                      <div class='{{$menu->is_dashboard?"is-dashboard":""}}' title="{{$menu->is_dashboard?'This is setted as Dashboard':''}}">
                         <i class='{{($menu->is_dashboard)?"icon-is-dashboard fa fa-dashboard":$menu->icon}}'></i> {{$menu->name}} <span class='pull-right'><a class='fa fa-edit' title='Edit'
                      ...
                      <ul>
                        @if($menu->children) @foreach($menu->children as $child) 
                         @php 
                            $privileges = DB::table('cms_menus_privileges') ->join('cms_privileges','cms_privileges.id','=','cms_menus_privileges.id_cms_privileges') ->where('id_cms_menus',$child->id)->pluck('cms_privileges.name')->toArray(); 
                            @endphp
                         <li data-id='{{$child->id}}' data-name='{{$child->name}}'>
                            <div class='{{$child->is_dashboard?"is-dashboard":""}}' title="{{$child->is_dashboard?'This is setted as Dashboard':''}}"><i class='{{($child->is_dashboard)?"icon-is-dashboard fa fa-dashboard":$child->icon}}'></i> {{$child->name}}
          ...
          <div class="kt-portlet kt-portlet--bordered">
            <div class="kt-portlet__head">
              <div class="kt-portlet__head-label">
                <div class="panel-heading">
                  <strong class="kt-font-danger kt-font-bold">Menu Order (Inactive)</strong>
                </div>
              </div>
            </div>
            <div class="kt-portlet__body">
